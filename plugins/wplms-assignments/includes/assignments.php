...
if (!class_exists('WPLMS_Assignments')){
    class WPLMS_Assignments
    {
        ...
        function assignment_submission_tab($tabs,$course_id){
            $tabs['assignment'] = sprintf(_x('Assignment Submissions <span>%d</span>','Assignment Submissions in course/admin/submissions','vibe'),self::get_assignment_submission_count($course_id));
            return $tabs;
        }
        ...
        function get_assignment_submissions($course_id){
            ...
            if(!empty($assignments)){
                ...
                <div class="submissions_form">
                    ...
                    <option value="all"> <?php echo __('All assignment', 'wplms-assignments') ?></option>
                    <?php
                ...
                <script>
                    jQuery(document).ready(function($){
                        $('#fetch_assignment_submissions').on('click',function(){
                            ...
                            if ($('#fetch_assignment').val() === 'all') {
                                $('#fetch_assignment option').each(function(){
                                    if ($(this).val() != 'all'){
                                        all_value.push(this.value);
                                    }
                                });
                            }else{
                                    all_value.push($('#fetch_assignment').val());
        ...
        function fetch_assignment_submissions(){
            ...
            if($_POST['status'] == 1){
                $assignment_submissions[] = $wpdb->get_results($wpdb->prepare ("SELECT meta_key,post_id FROM $wpdb->postmeta WHERE meta_value > '0' && post_id = %d",$assignment_id), ARRAY_A); // Internal Query
            }else{
                $assignment_submissions[] = $wpdb->get_results($wpdb->prepare ("SELECT meta_key,post_id FROM $wpdb->postmeta WHERE meta_value = '0' && post_id = %d",$assignment_id), ARRAY_A); // Internal Query    
            }
