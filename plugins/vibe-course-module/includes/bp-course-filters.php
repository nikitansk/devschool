...
class bp_course_filters{
    ...
    private function __construct(){
        ...
        add_filter('wplms_drive_value',array($this,'evaluate_course_drip'),99,4);
        add_filter('vibe_total_drip_duration',array($this,'total_drip_duration'),10,3);
...
//  bp_course_get_item_pagination
    ...
    function evaluate_course_drip($value,$pre_unit_id,$course_id,$unit_id){
    	...
    	if(vibe_validate($course_section_drip)){
				...
				if(vibe_validate($course_drip_duration_type)){
					...
					for($i=$k1;$i<=$k2;$i++){
						if(is_numeric($curriculum[$i]) && get_post_type($curriculum[$i]) == 'unit'){
							...
							$unit_duration_parameter = apply_filters('vibe_drip_duration_parameter',60,$curriculum[$i]);
