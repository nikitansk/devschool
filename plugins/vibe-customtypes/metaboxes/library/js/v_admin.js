...
(function() {
  ...
  var VibeBuilderPlugin = function() {
    YoastSEO.app.registerPlugin( 'VibeBuilderPlugin', { status: 'loading' } );
    this.bindElementEvents();
    this.builderContent='';
    this.loadBuilder( this.declareReady.bind( this ) );
  };

  VibeBuilderPlugin.prototype.declareReady = function() { console.log('ready');
    YoastSEO.app.pluginReady( 'VibeBuilderPlugin' );
    this.registerModifications();
  };


  VibeBuilderPlugin.prototype.declareReloaded = function() {
    YoastSEO.app.pluginReloaded( 'VibeBuilderPlugin' );
  };

  VibeBuilderPlugin.prototype.registerModifications = function() { 
    YoastSEO.app.registerModification( 'content', this.addVibeBuilder.bind( this ), 'VibeBuilderPlugin' );
  };

    VibeBuilderPlugin.prototype.bindElementEvents = function() {
        var contentElement = document.getElementById( 'content' ) || false;
        var that = this;

        if (contentElement) {
            contentElement.addEventListener( 'keydown', this.loadBuilder.bind( this, this.declareReloaded.bind( this ) ) );
            contentElement.addEventListener( 'change', this.loadBuilder.bind( this, this.declareReloaded.bind( this ) ) );
        }

        if( typeof tinyMCE !== 'undefined' && typeof tinyMCE.on === 'function' ) {
            tinyMCE.on( 'addEditor', function( e ) {
                e.editor.on( 'change', function() {
                    that.loadBuilder.bind( that, that.declareReloaded.bind( that ) )();
                });
  ...
  VibeBuilderPlugin.prototype.addVibeBuilder = function( data ) {

    console.log(this.builderContent+'##');
    data = data + 'lorem ipsum';//this.builderContent;
    console.log(data);
    return data;
  };

  VibeBuilderPlugin.prototype.loadBuilder = function( callback ) { console.log('loadbuilder');
        
        
        if ( typeof this.builderContent == 'undefined' || this.builderContent.length == 0 ) {
            this.parseBuilder();
        } else {
            callback();
        }
  ...
  VibeBuilderPlugin.prototype.parseBuilder = function( callback ) {
    
    if(typeof this.builderContent == 'undefined' || this.builderContent.length == 0){
        this.builderContent = $('#pagebuilder_generated_shortcode').val();
      };    
    }
