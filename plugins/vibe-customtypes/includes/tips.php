...
class WPLMS_tips{
    ...
    */
	...
	function submission_meta($user_id,$id){
		...
		$meta = $wpdb->get_var($wpdb->prepare("SELECT date_recorded FROM {$bp->activity->table_name} WHERE user_id = %d AND type = %s AND item_id = %d ORDER BY date_recorded DESC LIMIT 0,1",$user_id,'submit_quiz',$id));
	...
	function wplms_woocommerce_orders_link($loggedin_menu){
            if ( in_array( 'woocommerce/woocommerce.php', apply_filters( 'active_plugins', get_option( 'active_plugins' ) ) ) && is_user_logged_in()) {
