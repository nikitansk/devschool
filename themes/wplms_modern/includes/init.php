...
class Wplms_Modern_Init{
	...
	private function __construct(){
		...
		add_action('template_redirect',array($this,'restrict_edit_course_page'));
	}
	...
	function load_scripts(){
		if(defined('WPLMS_VERSION') && (WPLMS_VERSION == '2.0')){
			wp_enqueue_style( 'wplms-modern-css', get_stylesheet_directory_uri().'/assets/css/wplms_modern.min.css',array('wplms-style'));
		}else{
	...
	function wplms_modern_section($sections){
		...
		$sections[1]['fields'][] = array(
							'id' => 'alternate_logo',
							'type' => 'upload',
							'title' => __('Upload Alternate Logo', 'wplms_modern'), 
