...
<?php if ( bp_is_group_admin_screen( 'edit-details' ) ) : ?>
	...
	<p>
		<label for="group-notifiy-members">
			<input type="checkbox" name="group-notify-members" value="1" /> <?php _e( 'Notify group members of these changes via email', 'wplms_modern' ); ?>
		</label>
	</p>

	<?php do_action( 'bp_after_group_details_admin' ); ?>

	<p><input type="submit" value="<?php _e( 'Save Changes', 'wplms_modern' ); ?>" id="save" name="save" /></p>
...
<?php if ( bp_is_group_admin_screen( 'group-settings' ) ) : ?>
	...
	<?php if ( bp_is_active( 'forums' ) ) : ?>
		...
		<?php if ( bp_forums_is_installed_correctly() ) : ?>
			...
			<div class="checkbox">
				<label><input type="checkbox" name="group-show-forum" id="group-show-forum" value="1"<?php bp_group_show_forum_setting(); ?> /> <?php _e( 'Enable discussion forum', 'wplms_modern' ); ?></label>
			</div>
...
<?php if ( bp_is_group_admin_screen( 'group-avatar' ) ) : ?>
	...
	<?php if ( 'upload-image' == bp_get_avatar_admin_step() ) : ?>
			...
			<?php if ( bp_get_group_has_avatar() ) : ?>
				...
				<p><?php _e( "If you'd like to remove the existing avatar but not upload a new one, please use the delete avatar button.", 'wplms_modern' ); ?></p>

				<?php bp_button( array( 'id' => 'delete_group_avatar', 'component' => 'groups', 'wrapper_id' => 'delete-group-avatar-button', 'link_class' => 'edit', 'link_href' => bp_get_group_avatar_delete_link(), 'link_title' => __( 'Delete Avatar', 'wplms_modern' ), 'link_text' => __( 'Delete Avatar', 'wplms_modern' ) ) ); ?>
	...
	<?php if ( 'crop-image' == bp_get_avatar_admin_step() ) : ?>
		...
		<h3><?php _e( 'Crop Avatar', 'wplms_modern' ); ?></h3>
...
<?php if ( bp_is_group_admin_screen( 'manage-members' ) ) : ?>
	...
	<div class="bp-widget">
		<h4><?php _e( 'Administrators', 'wplms_modern' ); ?></h4>
		...
		<ul id="admins-list" class="item-list single-line">
			...
			<li>
				<?php echo bp_core_fetch_avatar( array( 'item_id' => bp_get_member_user_id(), 'type' => 'thumb', 'width' => 30, 'height' => 30, 'alt' => sprintf( __( 'Profile picture of %s', 'wplms_modern' ), bp_get_member_name() ) ) ); ?>
				<h5>
					<a href="<?php bp_member_permalink(); ?>"> <?php bp_member_name(); ?></a>
					<?php if ( count( bp_group_admin_ids( false, 'array' ) ) > 1 ) : ?>
					<span class="small">
						<a class="button confirm admin-demote-to-member" href="<?php bp_group_member_demote_link( bp_get_member_user_id() ); ?>"><?php _e( 'Demote to Member', 'wplms_modern' ); ?></a>
					</span>
					<?php endif; ?>
				</h5>
			</li>
	...
	<?php if ( bp_group_has_moderators() ) : ?>
		<div class="bp-widget">
			<h4><?php _e( 'Moderators', 'wplms_modern' ); ?></h4>
				...
				<ul id="mods-list" class="item-list single-line">
					...
					<li>
						...
						<h5>
							<a href="<?php bp_member_permalink(); ?>"> <?php bp_member_name(); ?></a>
							<span class="small">
								<a href="<?php bp_group_member_promote_admin_link( array( 'user_id' => bp_get_member_user_id() ) ); ?>" class="button confirm mod-promote-to-admin" title="<?php _e( 'Promote to Admin', 'wplms_modern' ); ?>"><?php _e( 'Promote to Admin', 'wplms_modern' ); ?></a>
								<a class="button confirm mod-demote-to-member" href="<?php bp_group_member_demote_link( bp_get_member_user_id() ); ?>"><?php _e( 'Demote to Member', 'wplms_modern' ); ?></a>
							</span>
						</h5>
					</li>
					<?php endwhile; ?>
	...
	<div class="bp-widget">
		<h4><?php _e("Members", "wplms_modern"); ?></h4>
				...
				<?php while ( bp_group_members() ) : bp_group_the_member(); ?>
					...
					<li class="<?php bp_group_member_css_class(); ?>">
						...
						<h5>
							...
							<?php if ( bp_get_group_member_is_banned() ) _e( '(banned)', 'wplms_modern'); ?>

							<span class="small">

							<?php if ( bp_get_group_member_is_banned() ) : ?>

								<a href="<?php bp_group_member_unban_link(); ?>" class="button confirm member-unban" title="<?php _e( 'Unban this member', 'wplms_modern' ); ?>"><?php _e( 'Remove Ban', 'wplms_modern' ); ?></a>

							<?php else : ?>

								<a href="<?php bp_group_member_ban_link(); ?>" class="button confirm member-ban" title="<?php _e( 'Kick and ban this member', 'wplms_modern' ); ?>"><?php _e( 'Kick &amp; Ban', 'wplms_modern' ); ?></a>
								<a href="<?php bp_group_member_promote_mod_link(); ?>" class="button confirm member-promote-to-mod" title="<?php _e( 'Promote to Mod', 'wplms_modern' ); ?>"><?php _e( 'Promote to Mod', 'wplms_modern' ); ?></a>
								<a href="<?php bp_group_member_promote_admin_link(); ?>" class="button confirm member-promote-to-admin" title="<?php _e( 'Promote to Admin', 'wplms_modern' ); ?>"><?php _e( 'Promote to Admin', 'wplms_modern' ); ?></a>

							<?php endif; ?>

								<a href="<?php bp_group_member_remove_link(); ?>" class="button confirm" title="<?php _e( 'Remove this member', 'wplms_modern' ); ?>"><?php _e( 'Remove from group', 'wplms_modern' ); ?></a>
		...
		<?php else: ?>
			...
			<div id="message" class="info">
				<p><?php _e( 'This group has no members.', 'wplms_modern' ); ?></p>
			</div>
...
<?php if ( bp_is_group_admin_screen( 'membership-requests' ) ) : ?>
			...
			<?php while ( bp_group_membership_requests() ) : bp_group_the_membership_request(); ?>
				...
				<li>
					...
					<div class="action">
						...
						<?php bp_button( array( 'id' => 'group_membership_accept', 'component' => 'groups', 'wrapper_class' => 'accept', 'link_href' => bp_get_group_request_accept_link(), 'link_title' => __( 'Accept', 'wplms_modern' ), 'link_text' => __( 'Accept', 'wplms_modern' ) ) ); ?>

						<?php bp_button( array( 'id' => 'group_membership_reject', 'component' => 'groups', 'wrapper_class' => 'reject', 'link_href' => bp_get_group_request_reject_link(), 'link_title' => __( 'Reject', 'wplms_modern' ), 'link_text' => __( 'Reject', 'wplms_modern' ) ) ); ?>
	...
	<?php else: ?>
		...
		<div id="message" class="info">
			<p><?php _e( 'There are no pending membership requests.', 'wplms_modern' ); ?></p>
		</div>
...
<?php if ( bp_is_group_admin_screen( 'delete-group' ) ) : ?>
	...
	<div id="message" class="info">
		<p><?php _e( 'WARNING: Deleting this group will completely remove ALL content associated with it. There is no way back, please be careful with this option.', 'wplms_modern' ); ?></p>
	</div>

	<label><input type="checkbox" name="delete-group-understand" id="delete-group-understand" value="1" onclick="if(this.checked) { document.getElementById('delete-group-button').disabled = ''; } else { document.getElementById('delete-group-button').disabled = 'disabled'; }" /> <?php _e( 'I understand the consequences of deleting this group.', 'wplms_modern' ); ?></label>

	<?php do_action( 'bp_after_group_delete_admin' ); ?>

	<div class="submit">
		<input type="submit" disabled="disabled" value="<?php _e( 'Delete Group', 'wplms_modern' ); ?>" id="delete-group-button" name="delete-group-button" />
	</div>
