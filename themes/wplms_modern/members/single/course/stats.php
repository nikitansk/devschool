...
if(!$cavg)$cavg= __('N/A','wplms_modern');

echo '<li>
		<div class="course_avatar">'.bp_course_get_avatar("id=$course->ID&size=thumbnail").'</div>
	  	<h4>'.bp_course_get_course_title("id=$course->ID").'</a><span><br />'.__('AVERAGE PERCENTAGE : ','wplms_modern').'<span>'.$cavg.'</span></span></h4>';
...
if($course_complete_status > 3){
	...
	if(($retakes - count($course_retakes)) > 0){
		...
		echo '<a class="retake_submit tip" title="'.__('Number of retakes remaining','wplms_modern').' : '.($retakes - count($course_retakes)).__(' out of ','wplms_modern').$retakes.'"><i class="icon-reload"></i></a>';
	...
	if(!isset($myavg) || $myavg == '')
		$myavg = __('TAKING','wplms_modern');


	echo '<strong>'.__('MY SCORE : ','wplms_modern').'<span>'.apply_filters('wplms_course_marks',$myavg.'/100',$course->ID).'</span></strong>';
	...
	foreach($curriculum as $c){
		if(is_numeric($c)){
			...
			if(get_post_type($c) == 'quiz'){
				...
				if(isset($myavg) && $myavg !=''){
					echo '<li>'.__('Average Marks in','wplms_modern').' '.get_the_title($c).' : '.$avg.'';
					echo '<strong>'.__('My Marks','wplms_modern').' : '.$myavg.' / '.$max.'</strong></li>';
				}
			}else{
	...
	if($user_course_status < time()){
		echo '<strong>'.__('Course Expired','wplms_modern').'</strong>';
	}
	switch($course_complete_status){
		case 1:
			echo '<strong>'.__('Not Started','wplms_modern').'</strong>';
		break;
		case 2:
			echo '<strong>'.__('In Progress','wplms_modern').'</strong>';
		break;
		case 3:
			echo '<strong>'.__('Under Evaluation','wplms_modern').'</strong>';
		break;
	}
}
