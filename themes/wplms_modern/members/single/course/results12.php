...
if(isset($_GET['action']) && !is_numeric($_GET['action'])){
		echo '<div id="message"><p>'.__('Invalid Results','wplms_modern').'</p></div>';
}else{
...
if(isset($_GET['action']) && $_GET['action']){ // Check Action
	...
	switch($post_type){
		case 'wplms-assignment':
		    ...
		    echo '<h3 class="heading">'.__('My Submission','wplms_modern').'</h3>';
		    ...
		    if(isset($answers) && is_array($answers) && count($answers)){
		        ...
		        echo '<div class="download_attachment"><a href="'.wp_get_attachment_url($attachment_id).'" target="_blank"><i class="icon-download-3"></i> '.__('Download Attachment','wplms_modern').'</a></div>';
		    }
		    ...
		    if(isset($remarks)){
		    	echo '<a href="'.trailingslashit( bp_loggedin_user_domain() . $bp->messages->slug . '/view/' . $remarks ).'" class="button right small">'.__('See Instructor Remarks','wplms_modern').'</a><span class="clearfix"></span>';
		    }
		    
		    echo '<div id="total_marks">'.__('Marks Obtained','wplms_modern').' <strong><span>'.$assignment_marks.'</span> / '.$total_assignment_marks.'</strong> </div>';
		    echo '</div>';
		break;
		case 'quiz':
			$quiz_id = $id;
			...
			$sum=$total_sum=0;
			echo '<div class="quiz_result"><h3 class="heading">'.get_the_title($quiz_id).((isset($course) && is_numeric($course))?'<a href="'.get_permalink($course).'" class="small_link">( &larr; '.__('BACK TO COURSE','wplms_modern').' )</a>':'').'<span class="right">'.social_sharing().' <a class="print_results"><i class="icon-printer-1"></i></a></span></h3>';
			if(count($questions)){
				...
				foreach($questions['ques'] as $key=>$question){
					...
					foreach($comments as $comment){ // This loop runs only once
					    ...
					    switch($type){
					      case 'truefalse': 
					      	$options = array( 0 => __('FALSE','wplms_modern'),1 =>__('TRUE','wplms_modern'));
					...
					if(isset($correct_answer) && $correct_answer !='' && isset($marks) && $marks !='' && $flag){
						...
						echo '<strong>';
						_e('Correct Answer :','wplms_modern');
						echo '<span>'.do_shortcode($ans).' '.((isset($explaination) && $explaination && strlen($explaination) > 5)?'<a class="show_explaination tip" title="'.__('View answer explanation','wplms_modern').'"></a>':'').'</span></strong>';
					}
						
					
					$total_sum=$total_sum+intval($questions['marks'][$key]);
					echo '<span> '.__('Total Marks :','wplms_modern').' '.$questions['marks'][$key].'</span>';

					if(isset($marks) && $marks !=''){
						if($marks > 0){
							echo '<span>'.__('MARKS OBTAINED','wplms_modern').' <i class="icon-check"></i> '.$marks.'</span>';
						}else{
							echo '<span>'.__('MARKS OBTAINED','wplms_modern').' <i class="icon-x"></i> '.$marks.'</span>';
						}
						$sum = $sum+intval($marks);
					}else{
						echo '<span>'.__('Marks Obtained','wplms_modern').' <i class="icon-alarm"></i></span>';
					}
				...
				echo '</ul>';
				echo '<div id="total_marks">'.__('Total Marks','wplms_modern').' <strong><span>'.$sum.'</span> / '.$total_sum.'</strong> </div></div>';
				...
				if(isset($retakes) && $retakes){
					...
					if( $retakes_left > 0){
						...
						echo '<input type="submit" name="initiate_retake" value="'.__('RETAKE QUIZ','wplms_modern').'" />';
						echo '<p id="retakes'.$retakes.'">'.__('Number of retakes ','wplms_modern').' : <strong>'.count($quiz_retakes).__(' out of ','wplms_modern').$retakes.'</strong></p>';
						wp_nonce_field('retake'.$user_id,'security');
						echo '</form>';
					}

					echo '<h4 id="prev_results"><a href="#">'.__('Previous Results for Quiz ','wplms_modern').'</a></h4>';
					...
					if(count($quiz_results) > 0){
						...
						foreach($quiz_results as $content){
							...
							echo '<div id="message"><p>'.__('Results not Available','wplms_modern').'</p></div>';
						}
					} // END Retakes
				}//END Questions	
			break;
		...
		?>
		<h3 class="heading"><?php _e('Quiz Results','wplms_modern'); ?></h3>
		...
		<li><i class="icon-task"></i>
			...
			<span><?php	
			if($value > 0){
				echo '<i class="icon-check"></i> '.__('Results Available','wplms_modern');
			}else{
				echo '<i class="icon-alarm"></i> '.__('Results Awaited','wplms_modern');
			}
			?></span>
			<span><?php
			...
			echo '<i class="icon-clock"></i> '.__('Submitted ','wplms_modern').tofriendlytime($diff) .__(' ago','wplms_modern');
		...
		<?php
			...
			<?php
				if($the_quiz->max_num_pages>1){?>
						...
						<div class="pag-count" id="course-dir-count-top">
							<?php echo sprintf(__('Viewing %d out of %d','wplms_modern'),$paged,$the_quiz->max_num_pages) ?>
						</div>
			...
			?>
			<h3 class="heading"><?php _e('Assignment Results','wplms_modern'); ?></h3>
			<ul class="quiz_results">	
		  	<?php
			...
			<?php
				if($the_assignment->max_num_pages>1){?>
						<div class="pagination">
						<div class="pag-count" id="course-dir-count-top">
							<?php echo sprintf(__('Viewing %d out of %d','wplms_modern'),$paged,$the_assignment->max_num_pages) ?>
						</div>
						<div class="pagination-links">
							<?php
